...
public class PrimalSimplex {
    ...
    public static PrimalSimplex min(int[] objectiveFunction, int[][] restrictions) {
        return new PrimalSimplex(objectiveFunction, restrictions);
    }

    private PrimalSimplex(int[] objectiveFunction, int[][] restrictions) {
        if (objectiveFunction == null || restrictions == null || objectiveFunction.length < 1 || restrictions.length < 1 || objectiveFunction.length != restrictions[0].length - 1) {
    ...
    public String solve() {
        ...
        int[] aObjectiveFunction = new int[objectiveFunction_.length + restrictions_.length];
        ...
        int[][] aRestrictions = new int[restrictions_.length][restrictions_[0].length + restrictions_.length];
        ...
        SimplexTableau aProblem = SimplexTableau.create(aObjectiveFunction, aBasis, aRestrictions);
        ...
        formatter.format("\nPROBLEMA INITIALA:");
        SimplexTableau iProblem = SimplexTableau.create(objectiveFunction_, aProblem);
