...
public final class SimplexTableau {
    ...
    public static SimplexTableau create(int[] objectiveFunction, int[] basis, int[][] restrictions) {
        ...
        for (int i = 1; i < z.length; i++) {
            z[0][0].add(Fraction.multiply(z[i][0], new Fraction(objectiveFunction[basis[i - 1]])));
        }
        for (int j = 1; j < z[0].length; j++) {
            z[0][j] = new Fraction(objectiveFunction[j - 1]).opposite();
            for (int i = 1; i < z.length; i++) {
                z[0][j].add(Fraction.multiply(z[i][j], new Fraction(objectiveFunction[basis[i - 1]])));
            }
        }
        return new SimplexTableau(objectiveFunction, basis, z);
    }

    public static SimplexTableau create(int[] objectiveFunction, SimplexTableau tableau) {
        int[] basis = tableau.basis_;
        Fraction[][] z = new Fraction[tableau.z_.length][tableau.z_[0].length - tableau.z_.length + 1];
        for (int i = 1; i < z.length; i++) {
            for (int j = 0; j < z[i].length; j++) {
        ...
        for (int i = 0; i < z_.length; i++) {
            for (int j = 0; j < z_[i].length; j++) {
                if (i != pivotRow && j != pivotColumn) {
                    z_[i][j] = Fraction.substract(Fraction.multiply(z_[pivotRow][pivotColumn], z_[i][j]),
                            Fraction.multiply(z_[pivotRow][j], z_[i][pivotColumn])).divide(z_[pivotRow][pivotColumn]);
                }
            }
        }
    ...
    private int pivotRow() {
        ...
        for (int i = 1; i < z_.length; i++) {
            if (z_[i][j].compareTo(Fraction.ZERO) > 0) {
                ...
                if (minValue.compareTo(Fraction.ZERO) == 0 || proportion.compareTo(minValue) < 0) {
                    minValue = proportion;
                    minPosition = i;
                }
            }
